...
def _(mo):
    ...
    ```css
    @layer core.color {
        /* Surface ramp — both bare .surface (auto-cascade via :has)
       and the explicit .surface-N variants compute --_surf-l from
       their own --depth value. */
    :where(body, .surface, .surface-0, .surface-1, .surface-2, .surface-3) {
      --_surf-top: calc(var(--cfg-surf-top-light) * (1 - var(--cfg-dark)) + var(--cfg-surf-top-dark) * var(--cfg-dark));
      --_surf-bot: calc(var(--cfg-surf-bot-light) * (1 - var(--cfg-dark)) + var(--cfg-surf-bot-dark) * var(--cfg-dark));
      --_t-stage:  clamp(0, calc(var(--depth) / 3), 1);
      --_surf-l:   calc((var(--_surf-top) + var(--_t-stage) * (var(--_surf-bot) - var(--_surf-top))) * 1%);
    ...
    /* Stage cascade — bare .surface infers depth from nesting.
       Variants stay outside this cascade so explicit stages don't
       inflate :has() inference on parents. */
    .surface                                          { --depth: 0 }
    .surface:has(.surface)                            { --depth: 1 }
    .surface:has(.surface .surface)                   { --depth: 2 }
    .surface:has(.surface .surface .surface)          { --depth: 3 }

    /* Explicit stage variants — pin a surface to a specific stage. */
    .surface-0 { --depth: 0 }
    .surface-1 { --depth: 1 }
    .surface-2 { --depth: 2 }
    .surface-3 { --depth: 3 }

    /* Paint — surfaces always paint their bg. */
...
def _(mo):
    ...
    @layer theme {
        ...
        body { transition: --cfg-type-scale calc(var(--cfg-motion) * 0.2s) ease-out; }
